...
desps_h = range(-10,11)
desps_v = range(-10,11)
metric_LPIPS = piq.LPIPS(reduction="none", mean=[0., 0., 0.], std=[1., 1., 1.,]).cuda()
metric_DISTS = piq.DISTS(reduction="none", mean=[0., 0., 0.], std=[1., 1., 1.,]).cuda()
metricas = {}
...
for desp_h in tqdm(desps_h):
    for desp_v in tqdm(desps_v):
        ...
        for img, label in dst_train:    
            ...
            img_desp = torch.from_numpy(img_desp).cuda()
            with torch.no_grad():
