...
history = clasificador.fit(dst_train.batch(128), epochs=config.epochs, validation_data = dst_test.batch(256), 
...
for desp_h in tqdm(desps_h):
    for desp_v in tqdm(desps_v):
        ...
        met_train = clasificador.evaluate(Xtrain_desplazado, Ytrain, batch_size=32, verbose=0, return_dict=True)
        met_test = clasificador.evaluate(Xtest_desplazado, Ytest, batch_size=32, verbose=0, return_dict=True)
